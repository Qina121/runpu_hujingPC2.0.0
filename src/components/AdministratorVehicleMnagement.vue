...
export default {
  ...
  methods: {
    ...
    showTableData() {
      const that = this
      this.$axios.get('https://api.huijingwuye6688.com/carManagement/PCSelectAll').then(function(res){
        that.tableData = []
    ...
    deleteData(index,item,rows) {
      ...
      // }
      const id = Number(item.id)
      const http = 'https://api.huijingwuye6688.com/carManagement/delete/'+id
      this.$axios.get(http).then(function(res){
        if(res.data.success) {
          that.$message({
